...
namespace App\Livewire\Admin\PendingListing;
...
class PendingListingIndex extends Component
{
    use WithPagination;
    ...
    {
        $model = Listing::with(['category'])->where('title','like','%'.$this->search.'%')
            ->where('is_approved','0')
            ->orderBy($this->sortBy, $this->sortDirection)
            ->paginate($this->paginate)->withQueryString();
        return view('livewire.admin.pending-listing.pending-listing-index',compact('model'));
    }
...
//        $this->selectAll = true;
    ...
    public function updateStatus($id,$status)
    {
        try{
            ...
            $listing->is_approved = $status;
            $listing->save();
            $this->alertSuccess('Listing Approved Successfully');
