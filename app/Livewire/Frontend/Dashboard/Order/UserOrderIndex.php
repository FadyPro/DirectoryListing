...
class UserOrderIndex extends Component
{
    ...
    {
        $model = Order::with(['user','package'])->where('user_id',Auth::user()->id)
            ->where('order_id','like','%'.$this->search.'%')
            ->orderBy($this->sortBy, $this->sortDirection)
            ->paginate($this->paginate)->withQueryString();
        return view('livewire.frontend.dashboard.order.user-order-index',compact('model'));
    }
    ...
    public function delete()
    {
        try{
            ...
            return redirect()->to(route('user.order.index'));
        }catch(\Exception $e){
