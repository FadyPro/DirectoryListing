<div>
                    ...
                    <form action="{{ route('listings') }}" method="GET">
                        <div class="listing_grid_sidbar">
                            <div class="sidebar_line">
                                <input wire:model="search" type="text" placeholder="Search" name="search" value="{{ request()->search }}">

                            </div>
                            <div wire:ignore class="sidebar_line_select">
                                <select wire:model="category" class="select_2" name="category">
                            ...
                            <div wire:ignore class="sidebar_line_select">
                                <select wire:model="location" class="select_2" name="location">
                            ...
                            <div class="wsus__pro_check">
                                @foreach ($amenities as $amenity)
                                    <div wire:ignore class="form-check">
                                        <input wire:model="amenity" @checked(in_array($amenity->slug, (request()->has('amenity') && is_array(request()->amenity)) ? request()->amenity : [])) class="form-check-input" type="checkbox" value="{{ $amenity->slug }}" name="amenity[]"
                        ...
                        @foreach ($listings as $listing)
                            <div class="col-xl-4 col-sm-6">
                                <div class="wsus__featured_single">
                                    ...
                                    <a class="map" wire:click.prevent="openModal({{ $listing->id }})"
    ...
    ==============================-->
    @if($isOpen)
        <livewire:frontend.pages.listing-modal :selectId="$selectId"/>
    @endif
    <!--=============================
            Modal END
   ==============================-->
